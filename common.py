...
#可学习权值融合 Learnable weights fusion
class LW_Fusion(nn.Module):
    def __init__(self, dimension=1):
        ...
        # 并且会向宿主模型注册该参数 成为其一部分 即model.parameters()会包含这个parameter
        # 从而在参数优化的时候可以自动一起优化
        self.wf = nn.Parameter(torch.ones(3, dtype=torch.float32), requires_grad=True)
        self.epsilon = 0.0001

    def forward(self, x):
        wf = self.wf
        weight = wf / (torch.sum(wf, dim=0) + self.epsilon)  # 将权重进行归一化
        # Fast normalized fusion
        x = [weight[0] * x[0], weight[1] * x[1], weight[2] * x[2]]
        return torch.cat(x, self.d)
#浅层语义特征补充模块 Shallow Semantic Feature Complementary Module
class SSFCM(nn.Module):
    def __init__(self, c1, c2):
        super(SSFCM, self).__init__()
        self.c1 = nn.Conv2d(c1, c1, 1, 1)
        self.c2 = nn.Conv2d(3 * c1, c2, 1, 1)
        self.dilated_conv1 = DilatedConv(c1, c1, 3, 1, 1)
        self.dilated_conv2 = DilatedConv(c1, c1, 3, 1, 2)
        self.dilated_conv3 = DilatedConv(c1, c1, 3, 1, 3)
        self.ema = EMA(3 * c1)
        self.fusion = LW_Fusion()

    def forward(self, x):

        d1_x = self.dilated_conv1(x)
        d2_x = self.dilated_conv2(x)
        d3_x = self.dilated_conv3(x)
        d_x = self.fusion([d1_x, d2_x, d3_x])
        y = self.ema(d_x)
        return self.c2(y)


#深层特征融合模块Deep Feature Fusion Module
class DFFM(nn.Module):
    def __init__(self, c1, dimension=1):
        super(DFFM, self).__init__()
        self.d = dimension
        self.df = nn.Parameter(torch.ones(3, dtype=torch.float32), requires_grad=True)
        self.epsilon = 0.0001
        self.TransposedConv = nn.ConvTranspose2d(c1, c1, 3, 2, autopad(3, None), 1)

    def forward(self, x):
        df = self.df
        weight = df / (torch.sum(df, dim=0) + self.epsilon)  # 将权重进行归一化
        # Fast normalized fusion
        x[0] = self.TransposedConv(x[0])
        x = [weight[0] * x[0], weight[1] * x[1], weight[2] * x[2]]
